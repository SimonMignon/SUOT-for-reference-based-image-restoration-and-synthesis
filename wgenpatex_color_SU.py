...
class semidual(nn.Module):
    ...
    """    
    def __init__(self, inputy, ρ, device=DEVICE, usekeops=False):
    ...
    def forward(self, inputx):
        if self.usekeops:
            ...
            loss = torch.mean(torch.sum((inputx-y[amin,:])**2,1)-self.psi[amin]) + torch.mean(self.ρ*(1-torch.exp(-self.psi/self.ρ)))
...
def optim_synthesis_SR(args,operator,lr_img,learn_img,lam,ρ,init=None,add_boundary=0,center=False,dim=2,n_splits=1):
    """
    Perform the texture synthesis of an examplar image
    """
    #target_img_name = args.target_image_path
    ...
    for s in range(n_scales):
        real_data = target_im2pat(target_downsampler[s].down_img, n_patches_out) # exctract at most n_patches_out patches from the downsampled target images 
        layer = semidual(real_data, ρ=ρ, usekeops=usekeops)
        semidual_loss.append(layer)
        if visu:
            imshow(target_downsampler[s].down_img)
    ...
    #fake_img = torch.rand(1, 3, nrow,ncol, device=DEVICE, requires_grad=True)
    if init is None:
        fake_img = torch.rand(1, 3, nrow,ncol, device=DEVICE).requires_grad_()
    else:
        fake_img = torch.tensor(init[np.newaxis,:,:,:],dtype=torch.float,device=DEVICE).requires_grad_()
    ...
    for it in range(n_iter_max):
        ...
        input_downsampler(fake_img.detach()) # evaluate on the current fake image
        for s in range(n_scales):            
            optim_psi = torch.optim.ASGD([semidual_loss[s].psi], lr=0.1, alpha=0.5, t0=1)
            for i in range(n_iter_psi):
                fake_data = input_im2pat(input_downsampler[s].down_img, n_patches_in)
                optim_psi.zero_grad()
                loss = -semidual_loss[s](fake_data)
