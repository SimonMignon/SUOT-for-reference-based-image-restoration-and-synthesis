...
class semidual(nn.Module):
    ...
    """    
    def __init__(self, inputy, ρ, usekeops=False):
    ...
    def forward(self, inputx):
        if self.usekeops:
            ...
            psi_mean = torch.mean(self.ρ*(1-torch.exp(-self.psi/self.ρ)))
        else:
...
def optim_synthesis(args,operator,lr_img,learn_img,lam,ρ,init=None,add_boundary=0,center=False,dim=2,n_splits=1):
    """
...
def optim_synthesis2(args,operator,lr_img,learn_img,lam,init=None,add_boundary=0,center=False,dim=2,n_splits=1):
    """
    Reconstruction of an image by minizing 
    .5*||operator(reconstruction)-lr_img||^2+lam*Wasserstein_patch_prior(reconstruction)
    Parameters:
    ...
    for it in range(1):
        ...
        for s in range(1):  
            optim_psi = torch.optim.ASGD([semidual_loss[s].psi], lr=1, alpha=0.5, t0=1)
            #optim_psi = torch.optim.ASGD([semidual_loss[s].psi], lr=0.01, alpha=0.75, t0=1)
            # set number of patches
            num_patches=np.prod([size-patch_size+1 for size in input_downsampler[s].down_img.shape[2:]])
            # iterate over update steps of psi
            for i in range(n_iter_psi):
