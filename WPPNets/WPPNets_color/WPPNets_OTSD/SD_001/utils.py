...
def save_img(tensor_img, name):
	...
	'''
	img = np.transpose(np.clip(tensor_img.squeeze().detach().cpu().numpy(),0,1), (1, 2, 0))
...
class semidual(nn.Module):
    ...
    def forward(self, inputx,patch_weights):
        if self.usekeops:
            ...
            #loss = torch.mean(torch.sum((inputx-y[amin,:])**2,1)-self.psi[amin]) + torch.sum(patch_weights*self.psi)/torch.sum(patch_weights) # transport équilibré 
            loss = torch.mean(torch.sum((inputx-y[amin,:])**2,1)-self.psi[amin]) + torch.sum(patch_weights*self.ρ*(1-torch.exp(-self.psi/self.ρ)))/torch.sum(patch_weights) # transport semi déséquilibré 
        else:
